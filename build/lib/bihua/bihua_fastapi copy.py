...
async def setup():
    ...
    if _star.messenger_admin_access_token == "":
        try:
            ...
            if not admin_access_token:
                ...
                load_status, profile_text = _resident.resident_profile_load()
                if load_status != CheckCrudStatus.SUCCESS: # initialization is needed.
                    profile_text = "Please enter agent profile text here..."
                    _resident.resident_profile_create_or_update(profile_text=profile_text)

                bihua_one_star.generate_resident_json_list()
                bihua_one_star.generate_group_json_list()
            ...
            # Always ensure client session is closed
            await client.close()
