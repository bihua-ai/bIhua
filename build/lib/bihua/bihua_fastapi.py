...
async def register_user(username: str = Body(..., embed=True), password: str = Body(..., embed=True)):
    ...
    try:
        ...
        register: RegisterStatus = await messenger_resident.register_user(username, password)
...
async def resident_profile_get(resident_id: str) -> Dict[str, Any]:
    try:
        ...
        profile_status, profile_data = messenger_resident.resident_text_profile_load(resident_id)
...
async def resident_profile_save(resident_id: str, profile_text: str = Body(..., embed=True)):
    try:
        ...
        profile_status, reason = messenger_resident.resident_text_profile_create_or_update(resident_id, profile_text)
...
async def document_delete(resident_id: str, file_name: str):
    try:
        ...
        delete_status, reason = messenger_resident.delete_resident_document(resident_id, file_name)
...
async def document_delete(group_id: str, file_name: str):
    try:
        ...
        # Perform the document delete operation
        delete_status, reason = messenger_group.delete_group_document(group_id, file_name)
...
async def get_document_list(id: str):
    try:
        ...
        if id.startswith("@"):
            ...
            data_status, profile_data = messenger_resident.get_uploaded_resident_document_names(resident_id)
        elif id.startswith("!"):
            group_id = id
            logger.debug(f"Group ID detected: {group_id}")
            data_status, profile_data = messenger_group.get_uploaded_group_document_names(group_id)
        else:
...
async def get_all_residents():
    try:
        ...
        messenger_resident.sync_and_save_all_resident_settings()
        list_status, resident_list_json = messenger_resident.get_resident_list()
...
async def get_all_groups():
    try:
        ...
        messenger_group.sync_and_save_groups_settings()
        list_status, group_list_json = messenger_group.get_group_list()
