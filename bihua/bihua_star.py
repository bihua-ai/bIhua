...
# do not remove config_manager, it loads .env from predefined location
...
class Star():
    ...
    def reload_settings(self):
        ...
        try:
            # load_dotenv("/opt/bihua/bihua/.env")
    ...
    def generate_resident_json_list(self):
        try:
            ...
            for resident_id in os.listdir(self.star_residents_data_home):
                resident_profile_full_path = os.path.join(self.star_residents_data_home, resident_id, self.resident_profile_subfolder)
                ...
                if len(resident_list) > 0:
                    with open(self.resident_list_json_path, 'w') as f:
                        json.dump(resident_list, f, indent=4)
                else:
                    with open(self.resident_list_json_path, 'w') as file:
                        file.write("{}")  # Create an empty JSON file

            logger.info(f"Resident list successfully saved to {self.resident_list_json_path}.")
    ...
    def generate_group_json_list(self):
        try:
            group_list = []
            # Iterate over resident directories
            for group_id in os.listdir(self.star_groups_data_home):
                group_profile_full_path = os.path.join(self.star_groups_data_home, group_id, self.group_profile_subfolder)
            ...
            logger.info(f"Group list successfully saved to {self.group_list_json_path}.")
