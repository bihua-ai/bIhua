...
import bihua_star, star_resident, utilities
    ...
    try:
        ...
        if response.status_code == 200:  # User created successfully
            ...
            if user_data_response.status_code == 200:
                ...
                star_resident.resident_settings_update(
                ...
                bihua_star.append_resident_json_list(resident_id=resident_id_to_be_created)
...
async def change_user_password(resident_id: str, new_password: str):
    try:
        ...
        if response.status_code == 200:
            ...
            star_resident.resident_settings_update(resident_id = resident_id, password = new_password)


            resident.profile_json_path
            bihua_star.update_resident_json_list(resident_id=resident_id)
...
async def change_user_display_name(resident_id: str, new_displayname: str) -> str:
    try:
        ...
        if response.status_code == 200:
            ...
            star_resident.resident_settings_update(resident_id=resident_id, display_name=new_displayname)
            bihua_star.update_resident_json_list(resident_id=resident_id)
...
async def change_user_type(resident_id, new_agent_type: str):
    try:
        ...
        if new_agent_type != msg_resident.agent:
            ...
            star_resident.resident_settings_update(resident_id=resident_id, agent=new_agent_type)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.put(url, json=data, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    star_resident.resident_settings_update(resident_id=resident_id, role=new_role)
                    bihua_star.update_resident_json_list(resident_id=resident_id)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.put(url, json=data, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    star_resident.resident_settings_update(resident_id=resident_id, state=new_state)
                    bihua_star.update_resident_json_list(resident_id=resident_id)
        ...
        async with aiofiles.open(avatar_file_path, "rb") as image_file:
            ...
            if isinstance(upload_response, UploadResponse):
                ...
                if isinstance(set_avatar_response, ProfileSetAvatarResponse):
                    ...
                    star_resident.resident_settings_update(resident_id, avatar_http_url=avatar_http_url)
                    bihua_star.update_resident_json_list(resident_id)
