...
import bihua_star, bihua_api, star_resident, star_group
from star_resident import Resident
from star_group import Group
...
async def setup():
    ...
    try:
        ...
        star_resident.resident_settings_save(_resident.settings)
        ...
        setting_status = star_resident.sync_and_save_all_resident_settings()
        ...
        _star.generate_resident_json_list() # overwrite existing if there is one

        setting_status = star_group.sync_and_save_groups_settings()
        if setting_status != CrudStatus.SUCCESS:
            logger.error(f"Error during setup: Failed  to collect and save resident's profiles.")
        _star.generate_group_json_list() # overwrite existing if there is one
...
async def admin_login(username: str = Body(..., embed=True), password: str = Body(..., embed=True)) -> Dict[str, str]:
    ...
    try:
        ...
        logger.exception(f"Unexpected error during login for username: {username} - {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def register_user(username: str = Body(..., embed=True), password: str = Body(..., embed=True)):
    ...
    try:
        ...
        register: RegisterStatus = await star_resident.register_user(username, password)
...
async def resident_profile_get(resident_id: str) -> Dict[str, Any]:
    try:
        ...
        profile_status, profile_data = star_resident.resident_text_profile_load(resident_id)
        ...
        if profile_status == CrudStatus.SUCCESS:
            ...
            logger.error(f"Failed. {profile_status} while fetching profile for {resident_id}.")
...
async def resident_profile_save(resident_id: str, profile_text: str = Body(..., embed=True)):
    try:
        ...
        profile_status, reason = star_resident.resident_text_profile_create_or_update(resident_id, profile_text)

        # Handle successful profile save
        if profile_status == CrudStatus.SUCCESS:
            logger.info(f"Profile for {resident_id} successfully saved.")
            return {"message": "success", "status": f"{status.HTTP_200_OK}"}

        # In case of unknown status
        else:
            logger.error(f"Unknown status while saving profile for {resident_id}.")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Unknown error occurred.")

    except Exception as e:
        # Log any unexpected errors
        logger.exception(f"Unexpected error while saving profile for {resident_id}: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error")
...
async def group_profile_get(group_id: str) -> Dict[str, Any]:
    try:
        ...
        _group = Group()
        profile_status, profile_data = _group.group_text_profile_load(group_id)
...
async def group_profile_save(group_id: str, profile_text: str = Body(..., embed=True)) -> Dict[str, Any]:
    try:
        ...
        _group = Group()
        profile_status, reason = _group.group_text_profile_create_or_update(group_id, profile_text)
...
async def document_delete(resident_id: str, file_name: str):
    try:
        ...
        delete_status, reason = star_resident.delete_resident_document(resident_id, file_name)
        
        # Handle different delete statuses
        if delete_status == CrudStatus.SUCCESS:
        ...
        elif delete_status == CrudStatus.ERROR:
        ...
        elif delete_status == CrudStatus.EXCEPTION:
            ...
            logger.error(f"Error status returned for document deletion: {delete_status}")
            return {
...
async def document_delete(group_id: str, file_name: str):
    try:
        ...
        # Perform the document delete operation
        delete_status, reason = star_group.delete_group_document(group_id, file_name)
...
######################
# It works for both group and residents
...
async def get_document_list(id: str):
    try:
        ...
        if id.startswith("@"):
            ...
            data_status, profile_data = star_resident.get_uploaded_resident_document_names(resident_id)
        elif id.startswith("!"):
            group_id = id
            logger.debug(f"Group ID detected: {group_id}")
            data_status, profile_data = star_group.get_uploaded_group_document_names(group_id)
        else:
...
@bihua.post("/api/resident_file_upload/{resident_id}", status_code=status.HTTP_200_OK)
async def upload_resident_file(resident_id: str, file_to_be_uploaded: UploadFile = File(...)) -> Dict:
    try:
        _star = Star()
        # Construct the directory and file paths
        resident_folder = os.path.join(_star.star_residents_data_home, resident_id, _star.resident_document_subfolder)
        os.makedirs(resident_folder, exist_ok=True)  # Ensure the directory exists

        original_filename = file_to_be_uploaded.filename
        file_destination_path = os.path.join(resident_folder, original_filename)
        
        # Check if file already exists
        if os.path.exists(file_destination_path):
            # Add a unique suffix before the file extension
            name, ext = os.path.splitext(original_filename)
            unique_suffix = f"_{uuid4().hex[:8]}"  # Generate a short unique identifier
            file_destination_path = os.path.join(resident_folder, f"{name}_{unique_suffix}{ext}")
        ...
        return_msg = {
            ...
            "status": f"{code}",
            "file": os.path.basename(file_destination_path)
        }

        logger.info(f"File '{return_msg['file']}' uploaded successfully for resident '{resident_id}'.")
        return return_msg

    except Exception as e:
        error_message = f"Failed to upload file '{file_to_be_uploaded.filename}' for resident '{resident_id}'. Error: {str(e)}"
...
@bihua.post("/api/group_file_upload/{group_id}", status_code=status.HTTP_200_OK)
async def upload_group_file(group_id: str, file_to_be_uploaded: UploadFile = File(...)) -> Dict:
    try:
        _star = Star()
        # Construct the directory and file paths
        group_folder = os.path.join(_star.star_groups_data_home, group_id, _star.group_document_subfolder)
        os.makedirs(group_folder, exist_ok=True)  # Ensure the directory exists

        original_filename = file_to_be_uploaded.filename
        file_destination_path = os.path.join(group_folder, original_filename)
        ...
        if os.path.exists(file_destination_path):
            ...
            name, ext = os.path.splitext(original_filename)
            unique_suffix = f"_{uuid4().hex[:8]}"  # Generate a short unique identifier
            file_destination_path = os.path.join(group_folder, f"{name}_{unique_suffix}{ext}")
        ...
        logger.info(f"File '{return_msg['file']}' uploaded successfully for group '{group_id}'.")
        return return_msg

    except Exception as e:
        error_message = f"Failed to upload file '{file_to_be_uploaded.filename}' for group '{group_id}'. Error: {str(e)}"
        logger.exception(error_message)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=error_message
...
async def get_all_residents():
    try:
        ...
        star_resident.sync_and_save_all_resident_settings()
        list_status, resident_list_json = star_resident.get_resident_list()
...
async def get_all_groups():
    try:
        ...
        star_group.sync_and_save_groups_settings()
        list_status, group_list_json = star_group.get_group_list()
